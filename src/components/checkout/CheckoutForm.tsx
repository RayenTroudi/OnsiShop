...
interface CheckoutFormProps {
  onSubmitSuccess?: (reservationId: string) => void;
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        if (onSubmitSuccess) {
          onSubmitSuccess(result.reservationId);
        } else {
    ...
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="mt-8 p-4 bg-blue-50 rounded-lg">
        <p className="text-sm text-blue-800">
          <strong>Note:</strong> This is a reservation system. No payment is required at this time. 
          We will contact you using the provided information to arrange delivery and payment.
        </p>
      </div>
    </div>
  );
