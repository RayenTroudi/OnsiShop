...
const hasValidCredentials = domain && key && 
  !domain.includes('demo-store') && 
  !key.includes('demo_token');

// Mock data for demo purposes
async function getMockData(query: string, variables?: any) {
  console.log('Using mock data for query:', query.substring(0, 50) + '...');
  
  // Only try to access database on server-side
  if (typeof window === 'undefined') {
    try {
      // Use server action to get mock data to avoid bundling Prisma on client
      const { getShopifyMockData } = await import('@/lib/server-actions/shopify-mock');
      return await getShopifyMockData(query, variables);
    } catch (e) {
      console.log('Error loading admin data from database:', e);
    }
  } else {
    console.log('Client-side request - using empty mock data');
  }
  
  // Fallback for client-side or when database fails
  if (query.includes('pageByHandle') || query.includes('getPage')) {
    return {
      status: 200,
      body: {
        data: {
...
export async function shopifyFetch<T>({
  ...
  if (!hasValidCredentials) {
    ...
    return await getMockData(query, variables) as { status: number; body: T };
  }
